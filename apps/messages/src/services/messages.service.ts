...
export class MessagesService implements OnModuleInit {
  ...
  async emit(req: MessageReq, mode?: GatewayMode) {
    try {
      await this.broadcastQueue.add(BULLMQ_JOB.DISPATCH, req);
    } catch (error) {
      this.logger.error(new EmitEventError(`Error emitting event to BULLMQ: ${BULLMQ_QUEUE.BROADCAST_MESSAGE}`, error));
    }

    try {
      if (mode === GatewayMode.PERSIST) await this.persistQueue.add(BULLMQ_JOB.PERSIST, req);
    } catch (error) {
      this.logger.error(new EmitEventError(`Error emitting event to BULLMQ: ${BULLMQ_QUEUE.PERSIST_MESSAGE}`, error));
    }

    try {
      if (mode === GatewayMode.VECTORIZE) await this.vectorizeQueue.add(BULLMQ_JOB.VECTORIZE, req);
    } catch (error) {
      this.logger.error(new EmitEventError(`Error emitting event to BULLMQ: ${BULLMQ_QUEUE.VECTORIZE_MESSAGE}`, error));
    }
  }
}
