...
<template>
  <div class="h-full">
    ...
    <div class="scrollbar-hide overflow-y-scroll h-[95%] mt-1">
      <!-- grid layout: 2 columns by default, 3 on xl -->
      <div :class="['grid gap-1 grid-cols-1', { 'grid-cols-2': vStore.atts.length > 1 }]">
        <AttachmentCard
          v-for="vision in vStore.atts"
          :key="(vision.file as File).name"
          :file="vision.file as File"
          class="bg-black/70 text-white w-full"
        >
          <template #icon-left>
            <IconifyJpeg v-if="isJpeg(vision.file as File)" class="size-8 m-0.5" />
            <IconifyWebp v-else-if="isWebp(vision.file as File)" class="size-8 m-0.5" />
            <IconifyPng v-else-if="isPng(vision.file as File)" class="size-8 m-0.5" />
          </template>
          ...
          <template #icon-right>
            ...
            <IconifyCancel
              ...
              @click="vStore.removeAtts(vision)"
            />
          </template>
