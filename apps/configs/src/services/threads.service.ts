...
export class ThreadsService implements OnModuleInit {
  ...
  async emit(req: ThreadReq, gateway?: GatewayMode) {
    try {
      await this.broadcastQueue.add(BULLMQ_JOB.MESSAGE, req);
      if (gateway === GatewayMode.PERSIST) await this.persistQueue.add(BULLMQ_JOB.PERSIST, req);
      if (gateway === GatewayMode.VECTORIZE) await this.vectorizeQueue.add(BULLMQ_JOB.VECTORIZE, req);
    } catch (error) {
