...
export class ThreadSchemaDocument extends Document {
   ...
   */
  @Prop({ type: String, enum: ThreadMode })
  mode?: ThreadMode;
   ...
   * @example "topic123"
   */
  @Prop({ type: Types.ObjectId, ref: TopicSchemaDocument.name })
  topicId: string;

  /**
   * The reference to the user who created the thread.\
   * This identifies the creator of the thread.
   *
   * @example "user123"
   */
  @Prop({ type: String, required: true })
