...
export class MessageSchemaDocument extends Document {
   ...
   */
  @Prop({ type: Types.ObjectId, ref: TopicSchemaDocument.name, required: true })
   ...
   */
  @Prop({ type: Types.ObjectId, ref: ThreadSchemaDocument.name, required: true })
   ...
   */
  @Prop({ type: Types.ObjectId, ref: MessageSchemaDocument.name })
   ...
   * @example "RESTRICTED"
   */
  @Prop({ type: String, enum: MessageMode })
  mode?: MessageMode;
   ...
   * Each flag has a label and counters for agree/disagree votes.
   *
   * @example [{ label: "MEME", agree: 3, disagree: 1 }]
   */
  @Prop({ type: [FLAG] })
...
MessageSchema.index({ topicId: 1 });
MessageSchema.index({ threadId: 1 });
MessageSchema.index({ topicId: 1, threadId: 1 });
MessageSchema.index({ topicId: 1, threadId: 1, publisherId: 1 });
