...
import { ThreadMode } from '@/constants/thread.constants';

export class ThreadReq {
  /**
   ...
   * This is used to track the creator of the thread.
   *
   * @example "user123"
   */
  @IsString()
  @ApiProperty({
    example: 'user123',
    description: 'The reference to the user who created the thread.',
  })
  createdBy: string;
  ...
  /**
   * The total number of replies or messages in this thread.\
  ...
  @ApiProperty({
    example: 15,
    description: 'The total number of replies or messages associated with this thread.',
    required: false,
  })
  replyCount?: number;
